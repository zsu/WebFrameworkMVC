...
namespace Web.Controllers.Api
{
    ...
    public class UserController : ApiController
    {
        ...
        public dynamic ExportToExcel([FromUri]JqGridSearchModel searchModel)
        {
            ...
            {
                searchModel.rows = 0;
                var data = GetQuery(Web.Infrastructure.Util.GetStatelessQuery<NhUserAccount>(),searchModel);
                var dataList = data.Items.Select(x => new
                {
                ...
                filePath = Web.Infrastructure.ExporterManager.Export("user", Web.Infrastructure.ExporterType.CSV, dataList.ToList(), "");
            }
            catch (Exception ex)
            {
        ...
        [HttpPut]
        public IHttpActionResult UpdateUserRoles(Guid id, [ModelBinder(typeof(FieldValueModelBinder))] UserRoleModel item)
        {
            StringBuilder message = new StringBuilder();
            if (id == default(Guid))
                return BadRequest("User id cannot be empty.");
        ...
        [HttpGet]
        public dynamic ExportToExcelUserRoles(Guid id, [FromUri]JqGridSearchModel searchModel)
        {
            string filePath = null;
            HttpResponseMessage result = null;
            try
            {
                ...
                filePath = Web.Infrastructure.ExporterManager.Export("userroles", Web.Infrastructure.ExporterType.CSV, dataList, "");
            }
            catch (Exception ex)
            {
                return Web.Infrastructure.Util.DisplayExportError(ex);
        ...
        private Web.Infrastructure.GridModel<NhUserAccount> GetQuery(IQueryable<NhUserAccount> query,[FromUri] JqGridSearchModel searchModel, int maxRecords = Constants.DEFAULT_MAX_RECORDS_RETURN)
        {
            if (!User.IsInRole(Constants.ROLE_ADMIN))
            {
